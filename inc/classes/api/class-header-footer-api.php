...
class Header_Footer_Api
{
	...
	public function get_custom_logo_url($key)
	{
		...
		// Controleer of $custom_logo_id een geldige waarde heeft
		if ($custom_logo_id) {
			$image = wp_get_attachment_image_src($custom_logo_id, 'full');

			// Controleer of $image een array is en toegang tot het eerste element veilig is
			if (is_array($image) && isset($image[0])) {
				return $image[0];
			}
		}

		// Geef een lege string terug als er geen geldige afbeelding is gevonden
		return '';
	}
